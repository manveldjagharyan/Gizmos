...
function newMessage(description) {
    ...
    message.append(closeMessage, text);
...
const showLinks = () => {
    ...
    shareLinks.style.animation = 'shareLinksOn 0.3s ease forwards';
};

const hideLinks = () => {
    shareLinks.style.animation = 'shareLinksOff 0.3s ease forwards';
    setTimeout(() => {
        if (!shareLinks.matches(':hover') && !shareButton.matches(':hover')) {
            shareLinks.style.display = 'none';
        }
    }, 300);
};

shareButton.onmouseover = showLinks;
shareLinks.onmouseover = showLinks;

shareButton.onmouseleave = hideLinks;
shareLinks.onmouseleave = hideLinks;